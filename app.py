...
# [수정] 버튼 클릭 시 세션 상태와 위젯 키(Key)를 강제 동기화
...
def render_calendar_tab():
    ...
    # [복구] 월간 현황은 Schedules DB만 표시 (기존 유지)
    ...
    def get_day_html(day, is_horiz=True):
        ...
        if not is_horiz and not today_sch.empty:
            ...
            for _, row in today_sch.iterrows():
                ...
                orig = calculate_auto_shift(get_group_from_dict(history_dict, row['name'], d_str), d_str)
                orig_mk = ""
...
def main():
    ...
    # [수정] NameError 방지를 위해 함수 정의가 다 끝난 후 실행
