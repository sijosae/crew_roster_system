...
def _render_calendar_tab_unsafe():
    ...
    def get_day_html(day, is_horiz=True):
        ...
        # [중요 수정] div 닫는 태그를 조건문 밖으로 빼서 항상 닫히도록 함
        html += '</div>'
        return html
...
def render_individual_calendar_tab():
    ...
    with c_yr: 
        # index 계산
        years = list(range(2023, now.year + 3))
        try: y_idx = years.index(st.session_state.indiv_view_year)
        except: y_idx = 1
        st.selectbox("년도", years, index=y_idx, key='sb_ind_year', label_visibility="collapsed")
