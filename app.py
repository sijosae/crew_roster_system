    ...
    for start_row in date_rows:
        ...
        for side in cols_map:
            ...
            for r_offset in range(3, 75):
                ...
                if not (current_route and current_seq):
                    continue
                ...
                if is_reduction:
                    if am_fix:
                        extracted_data.append({
                            'date': current_date, 'name': am_fix, 'shift': '감차휴무',
                            'route': current_route, 'seq': current_seq, 'car': '감차',
                            'is_sub': 'FALSE', 'orig_fix': am_fix, 'updated_at': get_kst_now().strftime("%Y-%m-%d %H:%M:%S")
                        })
                    if pm_fix:
                        extracted_data.append({
                            'date': current_date, 'name': pm_fix, 'shift': '감차휴무',
                            'route': current_route, 'seq': current_seq, 'car': '감차',
                            'is_sub': 'FALSE', 'orig_fix': pm_fix, 'updated_at': get_kst_now().strftime("%Y-%m-%d %H:%M:%S")
                        })
                    continue
                ...
                if am_final:
                    extracted_data.append({
                        ...
                        'is_sub': str(bool(am_sub)).upper(), 'orig_fix': am_fix, 'updated_at': get_kst_now().strftime("%Y-%m-%d %H:%M:%S")
                    })
                ...
                if pm_final:
                    extracted_data.append({
                        ...
                        'route': current_route, 'seq': current_seq, 'car': current_car,
                        'is_sub': str(bool(pm_sub)).upper(), 'orig_fix': pm_fix, 'updated_at': get_kst_now().strftime("%Y-%m-%d %H:%M:%S")
                    })
...
def _render_calendar_tab_unsafe():
    ...
    with c2:
        ...
        st.selectbox("년도", year_range, index=year_range.index(st.session_state.view_year), key='sb_view_year', label_visibility="collapsed")
    # 동기화 로직
    ...
    with c4:
        ...
        st.selectbox("월", month_range, index=month_range.index(st.session_state.view_month), key='sb_view_month', label_visibility="collapsed")
    # 동기화 로직
    ...
    # [수정] 데이터 불러오기 및 날짜 형식 정규화 (이 부분이 핵심 수정 사항)
    df = load_data("schedules")
    if not df.empty and 'date' in df.columns:
        # 날짜 컬럼을 강제로 YYYY-MM-DD 형식으로 통일
...
def render_individual_calendar_tab():
    ...
    # [핵심 수정] 스케쥴 데이터 불러오기 및 정제 (날짜 형식 통일)
    df_plan = load_data("schedules")

    # 데이터가 있다면 컬럼 이름 공백 제거 및 날짜/이름 정규화
    if not df_plan.empty:
        df_plan.columns = df_plan.columns.str.strip() # 컬럼명 공백 제거

        if 'date' in df_plan.columns:
            # 날짜를 표준 형식 YYYY-MM-DD로 변환 (2025-1-1 등 비표준 형식 대응)
            df_plan['date'] = pd.to_datetime(df_plan['date'], errors='coerce').dt.strftime("%Y-%m-%d")

        if 'name' in df_plan.columns:
            # 이름 앞뒤 공백 제거
            df_plan['name'] = df_plan['name'].astype(str).str.strip()

    # 데이터가 없거나 필수 컬럼이 없는 경우 빈 프레임 생성
    ...
    # [수정] 초기화 로직 강화 (세션 스테이트가 없을 때만 오늘 날짜 대입)
    ...
    with c_yr:
        # [핵심] Selectbox 값과 indiv_view_year 동기화
        year_range_ind = range(2023, now.year + 3)
        selected_year = st.selectbox("년도", year_range_ind, index=year_range_ind.index(st.session_state.indiv_view_year), key='sb_ind_year', label_visibility="collapsed")
    ...
    with c_mo:
        # [핵심] Selectbox 값과 indiv_view_month 동기화
        month_range_ind = range(1, 13)
        selected_month = st.selectbox("월", month_range_ind, index=month_range_ind.index(st.session_state.indiv_view_month), key='sb_ind_month', label_visibility="collapsed")
    ...
    if target:
        ...
        # [확인] 날짜 컬럼 형식 통일로 인해 startstartswith가 정상 작동함
            ...
            for i, day in enumerate(week):
                with cols[i]:
                    if day == 0: st.write("")
                        ...
                        if not p_work.empty:
                            w_row = p_work.iloc[0]
                            is_sub_str = str(w_row.get('is_sub', 'FALSE')).upper()
                            is_sub = (is_sub_str == 'TRUE' or is_sub_str == 'Y')

                            if w_row['shift'] == '감차휴무':
                                cell_bg = get_type_color('감차휴무')
                                txt = f"<span style='color:white; font-weight:bold;'>감차휴무</span><br><span style='font-size:9px; color:white; opacity:0.8;'>({w_row['route']}-{w_row['seq']})</span>"
                            else:
                                if w_row['shift'] == '오전': cell_bg = get_type_color('실제근무_본인')
                                elif w_row['shift'] == '오후': cell_bg = "#e53935" # 진한 빨강
                                if is_sub: cell_bg = get_type_color('실제근무_대운')
                            ...
                            txt = f"<span style='color:white;'>{t}</span>"
                        else:
