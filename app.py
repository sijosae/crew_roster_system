...
def render_calendar_tab():
    ...
    with c2: 
        ...
        new_y = st.selectbox("년도", years, index=idx, key='sb_view_year', label_visibility="collapsed")
        if new_y != st.session_state.view_year: st.session_state.view_year = new_y; st.rerun()

    with c3: st.markdown("<div style='padding-top:10px; text-align:right; font-weight:bold;'>월:</div>", unsafe_allow_html=True)
    with c4: 
        new_m = st.selectbox("월", range(1, 13), index=st.session_state.view_month-1, key='sb_view_month', label_visibility="collapsed")
        if new_m != st.session_state.view_month: st.session_state.view_month = new_m; st.rerun()
