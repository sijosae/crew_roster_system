...
@@ -36,6 +38,7 @@ def next_cal_callback():
st.session_state.view_month = 1
else:
st.session_state.view_month += 1
    # Selectbox 동기화
...
@@ -45,6 +48,7 @@ def prev_month_indiv():
...
@@ -54,27 +58,41 @@ def next_month_indiv():
st.session_state.indiv_view_month = 1
else:
st.session_state.indiv_view_month += 1
    # Selectbox 동기화
...
@@ -85,15 +103,20 @@ def log_login_access(username, name):
...
@@ -109,137 +132,332 @@ def load_data(sheet_name):
...
try:
@@ -248,47 +466,60 @@ def calculate_auto_shift(group_name, target_date_str):
...
c_recs = [curr]
@@ -304,120 +535,289 @@ def calculate_layout_rows(df_month):
...
@@ -426,59 +826,101 @@ def get_day_html(day, is_horiz=True):
...
@@ -489,43 +931,427 @@ def get_day_html(day, is_horiz=True):
...
df_plan = load_data("schedules")
    ...
    with c_yr: 
        ...
        selected_year = st.selectbox("년도", range(2023, now.year + 3), index=range(2023, now.year + 3).index(st.session_state.indiv_view_year), key='sb_ind_year', label_visibility="collapsed")
    ...
    with c_mo: 
        ...
        selected_month = st.selectbox("월", range(1, 13), index=st.session_state.indiv_view_month - 1, key='sb_ind_month', label_visibility="collapsed")
    ...
    if target:
        ...
        # [확인] 날짜 컬럼 형식 통일로 인해 startswith가 정상 작동함
